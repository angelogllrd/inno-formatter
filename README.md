...
[Inno Setup](https://jrsoftware.org/isinfo.php) genera un script previo a la compilación **para obtener el instalador de nuestro programa Python**, pero antes de compilar suele ser necesario realizar varias modificaciones manuales para que el instalador funcione correctamente.
...
Para instalaciones silenciosas (`/SILENT`):

2. **Ícono en el escritorio por defecto:** Elimina el flag `unchecked` para que la opción de crear un acceso directo en el escritorio ya venga marcada.
3. **Ejecución al terminar instalación:** Quita la restricción `skipifsilent` para que el programa se inicie automáticamente al terminar la instalación cuando esta es silenciosa.
